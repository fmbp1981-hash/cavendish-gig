...
import {
  Loader2, Plus, CalendarCheck, AlertTriangle, CheckCircle2,
  ChevronRight, Clock, BookOpen, Trash2,
} from "lucide-react";
import { format, parseISO, isBefore, isThisMonth, addMonths } from "date-fns";
...
import {
  ...
  STATUS_COR,
  isAtrasada,
  useComplianceObrigacoes,
  useCriarObrigacao,
  useConcluirObrigacao,
  useExcluirObrigacao,
...
function ObrigacaoFormDialog({
  ...
  const handleSave = async () => {
    ...
    await criar.mutateAsync({
      ...
      lei_referencia: lei || undefined,
      orgao_regulador: orgao || undefined,
      periodicidade,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Nova Obrigação Regulatória</DialogTitle>
        </DialogHeader>
        <div className="space-y-3 py-2">
          <div className="space-y-1.5">
            <Label>Título *</Label>
            <Input placeholder="Ex: Revisão anual do Código de Ética" value={titulo} onChange={e => setTitulo(e.target.value)} />
          </div>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              <Label>Lei/Norma</Label>
              <Input placeholder="Ex: Lei 12.846/2013" value={lei} onChange={e => setLei(e.target.value)} />
            </div>
            <div className="space-y-1.5">
              <Label>Órgão Regulador</Label>
              <Input placeholder="Ex: CGU, ANPD, CVM" value={orgao} onChange={e => setOrgao(e.target.value)} />
            </div>
          </div>
          <div className="space-y-1.5">
            <Label>Descrição</Label>
            <Textarea placeholder="Detalhes da obrigação..." value={descricao} onChange={e => setDescricao(e.target.value)} rows={2} />
          </div>
        </div>
        <DialogFooter>
...
function ObrigacaoCard({
  ...
  obrigacao: ComplianceObrigacao;
  onConcluir: () => void;
  onExcluir: () => void;
}) {
  const atrasada = isAtrasada(obrigacao);
  const diasRestantes = Math.ceil(
    (parseISO(obrigacao.proxima_data).getTime() - Date.now()) / (1000 * 60 * 60 * 24)
  );
  ...
  return (
    <Card className={cn(
      "transition-colors",
      obrigacao.status === "concluida" && "opacity-60",
      atrasada && "border-red-400/50",
      urgente && !atrasada && "border-orange-400/40",
    )}>
          ...
          <div className="min-w-0 flex-1 space-y-1">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {obrigacao.orgao_regulador && (
                <Badge variant="secondary" className="text-[10px]">{obrigacao.orgao_regulador}</Badge>
              )}
              <Badge variant="outline" className={cn("text-[10px]", STATUS_COR[obrigacao.status])}>
                {STATUS_LABEL[obrigacao.status]}
              </Badge>
            </div>
            {obrigacao.lei_referencia && (
              <p className="text-xs text-muted-foreground">{obrigacao.lei_referencia}</p>
            )}
            <div className="flex items-center gap-3 text-xs flex-wrap">
              <span className={cn(
                ...
                {atrasada
                  ? `⚠️ ${Math.abs(diasRestantes)}d atrasada`
                  : diasRestantes === 0
                  ? "⚠️ Vence hoje"
                  : diasRestantes < 0
              ...
              <span className="text-muted-foreground">
                {format(parseISO(obrigacao.proxima_data), "dd/MM/yyyy", { locale: ptBR })}
              </span>
              <Badge variant="outline" className="text-[10px]">
                {PERIODO_LABEL[obrigacao.periodicidade]}
              </Badge>
            </div>
          </div>
...
export default function ComplianceCalendar() {
  ...
  const orgaos = [...new Set((obrigacoes ?? [])
    .map(o => o.orgao_regulador)
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">
            Agenda de obrigações legais e regulatórias com alertas de vencimento
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Button size="sm" onClick={() => setFormOpen(true)}>
            <Plus className="h-4 w-4 mr-1.5" />Nova obrigação
          </Button>
        </div>
          ...
          <div className="space-y-2">
            {obrigacoesFiltradas.map(o => (
              <ObrigacaoCard
                key={o.id}
                obrigacao={o}
                onConcluir={() => concluir.mutate(o)}
                onExcluir={() => {
                  if (window.confirm(`Remover "${o.titulo}"?`)) excluir.mutate(o.id);
                }}
              />
            ))}
          </div>
        ...
        <ObrigacaoFormDialog
          ...
          organizacaoId={selectedOrg || undefined}
        />
      </div>
    </ConsultorLayout>
  );
